...
export function ChatMessages({ messages, chatId, }: { messages: Message[], chatId: string }) {
    ...
    return <div>
        <div className="items-center align-middle">
            {message.map((x) => {
                i += 1;
                return <div className={`chat chat-${pos[i % 2 - 1]}`} key={x.id}>
                    ...
                    <div className="chat-header">
                        ...
                        <time className="text-xs opacity-50">{x.created_at.getHours()}:{x.created_at.getMinutes()}</time>
                    </div>
